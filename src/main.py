# Main snake game file. Arrow keys move the snake. P = pause, S = speed toggle, A = auto-play.
...
#   A = auto-play on/off 
#   F = switch solver (A* / BFS)
#   Q = queue mode on/off (stores arrow presses)
#   D = show/hide debug info
#   B = run small sort benchmark (merge vs quick sort)
#   P = pause / resume
...
def main():
    ...
    pygame.init() # setup window and clock
    screen = pygame.display.set_mode((WINDOW_W, WINDOW_H))
    pygame.display.set_caption("Unit 24 Snake")
    clock = pygame.time.Clock()

                                          
    pygame.font.init() # font for hud text
    font = pygame.font.Font(None, 24)

                                                      
    snake = Snake((GRID_W // 2, GRID_H // 2)) # start in centre of grid
    food = random_empty(set(snake.body)) # random food spawn point position
    score = 0
    ...
    while running:
        ...
        if not PAUSED: #move snake and check scoring/collision
        ...
        trace_push(callstack, "draw") # draw grid snake food path and hud text
        screen.fill(BLACK)
        draw_grid(screen)
        ...
        speed_mult = 2 if SPEED == "FAST" else 1 # Limit FPS so the game speed is controlled
